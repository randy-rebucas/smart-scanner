...
import ScannedData from "@/models/ScannedData";
...
export async function GET() {
    try {
        ...
        await dbConnect();
        const docs = await ScannedData.find({ user: session.user.email }).sort({ createdAt: -1 });
...
export async function POST(req: NextRequest) {
    try {
        ...
        const doc = await ScannedData.create({
            ...body,
